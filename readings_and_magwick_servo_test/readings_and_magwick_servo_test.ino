...
float offset_gyroX = 0.010321; 
float offset_gyroY = 0.011863;
float offset_gyroZ = -0.021387;
...
void loop() {
  ...
  // Map the potentiometer value from 0-1023 to 0-255 (brightness range for PWM)
  pitch_center = map(potValue, 0, 1023, 45, 135);

  myservo.write(pitch_center+5-pitch);
  myotherservo.write(roll_center+roll);
...
void gyro_integration(){
  ...
  unsigned long elapsed_microseconds = current_time - last_angle_time;
  unsigned long elapsed_seconds = elapsed_microseconds / 1000000; // Convert microseconds to seconds

  // Remainder in milliseconds
  unsigned long remainder_microseconds = elapsed_microseconds % 1000000;

  // Convert remainder to fractional seconds with higher precision
  unsigned long precision_seconds = (remainder_microseconds * 1000) / 1000000;

  // Combine whole seconds and precision seconds
  float elapsed_time = elapsed_seconds + (float)precision_seconds / 1000.0;

  GyroX *= 57.29577951308232;
  GyroY *= 57.29577951308232;
  GyroZ *= 57.29577951308232;


  angleX += GyroX*elapsed_time;
  angleY += GyroY*elapsed_time;
  angleZ += GyroZ*elapsed_time; //computed angles in degrees
  last_angle_time = micros();

  // Update the FusionAHRS with gyroscope and accelerometer data
  const FusionVector gyroscope = {GyroX, GyroY, GyroZ}; // Gyroscope data in degrees/s
